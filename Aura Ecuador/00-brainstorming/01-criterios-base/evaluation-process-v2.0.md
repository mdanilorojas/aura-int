# Proceso de evaluacion de ideas v2.0

> **Estado:** fuente para visual HTML  
> **Version:** Evaluation Process v2.0 · 2026-05-24  
...
> **Cambios v2.0:** dos capas nuevas (Capa 8 monitoreo semanal y Capa 9 cadencia de re-evaluacion) que cierran el loop entre scoring inicial y operacion continua. Tambien se reconoce que la lista de clientes nominales y el playbook de venta sin empresa son entregables que el sistema debe producir, no opcionales.
...
## Capa 8: monitoreo semanal (leading indicators)

El ranking inicial produce decisiones de gran escala (qué idea lanzar primero, dónde poner foco). Pero entre ranking y ranking, el equipo necesita saber semana a semana si va por buen camino. Los pivots del ranking son lagging (mes 2, mes 6); sin leading indicators, la reaccion llega tarde.

Cada viernes, en 15 minutos, se actualizan cinco indicadores:

- mensajes outbound enviados/semana;
- tasa de respuesta cold outreach;
- reuniones de descubrimiento celebradas/semana;
- propuestas enviadas/mes;
- LOIs y contratos firmados/mes.

Umbrales objetivo y disparadores de ajuste viven en `criterios-base-ideas.md` seccion 9.1. El dashboard se mantiene en un archivo simple (Notion o Sheets), con responsable claro (Bastien).

## Capa 9: cadencia de re-evaluacion

El ranking no es un evento unico. El sistema se mantiene vivo con tres cadencias acopladas:

| Cadencia | Frecuencia | Responsable | Output |
|---|---|---|---|
| Semanal | viernes | Bastien | dashboard de leading indicators |
| Mensual | primer lunes | Danilo | nota corta de cambios materiales en archivo activo |
| Trimestral | cada 3 meses | todo el equipo | nuevo `ranking-ideas-vX.Y.md` y `.html` con re-puntuacion |

Detalle de cada cadencia en `criterios-base-ideas.md` seccion 9. Cualquier cambio regulatorio mayor o cierre de cliente importante puede adelantar el re-ranking trimestral.

## Proximo entregable

Despues de aprobar este sistema, el siguiente paso es generar el ranking de ideas:

1. Releer criterios base.
2. Revisar las ideas contra los 40 criterios.
3. Aplicar filtros de descarte (incluyendo nuevos: jurisdiccion del comprador, lista de prospectos nombrada).
4. Re-evaluar las 18 ideas con sub-criterio de concentracion de riesgo (4.8.b).
...
8. Producir lista de clientes nominales del top 5 (asset obligatorio v2.0).
9. Producir playbook actualizado de venta sin empresa (si aplica).
10. Generar `ranking-ideas-vX.Y.md`.
11. Generar `ranking-ideas-vX.Y.html`.
12. Activar monitoreo semanal (Capa 8) y agendar proximo re-ranking trimestral (Capa 9).
